...
BLINK_TIME = {
    'BLINK_START_TEX_SHORT': 3,
    'BLINK_END_TEX_SHORT' : 5,
    'BLINK_START_TEX_LONG' : 11,
    'BLINK_END_TEX_LONG' : 12,
    'BLINK_START_PIC_PIC' : 1,
    'BLINK_END_PIC_PIC' : 3,
    'BLINK_START_VID_SHORT' :2,
    'BLINK_END_VID_SHORT' : 4,
    'BLINK_START_VID_LONG' : 5,
    'BLINK_END_VID_LONG' : 7,
    'BLINK_START_BAS_BASELINE':1,
    'BLINK_END_BAS_BASELINE' : 3,
}
...
def compute_correlation_coeff(feature_list):
    ...
    if (corr>0.3 or corr < -0.3) and p_val<0.05:
        print "corr=%s, p_val=%s" .format(corr,p_val)
        plt.scatter(X,Y)
...
def compute_correlation_coeff_feat_label(feature_list,label_data ):
    ...
    if (corr>0.3 or corr < -0.3) and p_val<0.05:
        print "corr="+str(corr)+" p_val="+str(p_val)
        plt.scatter(X,Y)
        plt.ylabel("Popular/Non Popular")
        plt.xlabel("Frequency")  
        plt.show() 
...
def generate_blink_data_by_subject(id):
    all_slide_data = {}
    all_users_att_med  = {}
    all_users_alpha_beta  = {}
...
if __name__ == "__main__":
    ...
    for subject_id in range(101,130):
        user_id = str(subject_id)
        print "\n\n\n\n\n"
        print "subject: " + user_id
        generate_blink_data_by_subject(user_id)
        compute_correlations_blink()
